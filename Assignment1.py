...
def main():
    ...
    return runner(test_set, train_set, grid),manhattan_distance(test_set, train_set, grid)
...
def runner(test_set, train_set, grid):
    ...
    for k in k_values:
        ...
        plt.title("KNN with k = {} Train Error ={} Test Error ={}".format(k, round(1 - knn.score(tr_data, tr_label), 5),
                                                                          round(1 - knn.score(te_data, te_label), 5)))
        plt.savefig('{}.jpg'.format(k), dpi=300)

def manhattan_distance(test_set, train_set, grid):
    tr_data = train_set[['X', "Y"]].to_numpy()
    te_data = test_set[['X', "Y"]].to_numpy()
    tr_label = train_set[['label']].to_numpy()
    te_label = test_set[['label']].to_numpy()
    ...
    plt.scatter(train_set["X"], train_set["Y"], c=colors, marker='o')
    plt.scatter(test_set["X"], test_set["Y"], c=colors2, marker='+')
    plt.title("KNN Manhattan with k = 30 Train Error ={} Test Error ={}".format( round(1 - knn.score(tr_data, tr_label), 5),
                                                                      round(1 - knn.score(te_data, te_label), 5)))
